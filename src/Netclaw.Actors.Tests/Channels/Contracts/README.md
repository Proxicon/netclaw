...
| `TeamsSessionBindingContractTests` | 8 | Yes | Teams fixture for the common SDK-free binding lifecycle: ingress classification, shared approval response outcomes, recovery, pipeline reinitialization, delivery failure, and fail-loud feedback. Opaque card correlation, nonce/replay, and expiry remain focused Teams transport tests. |
| `TeamsPersonalRoutingTests` | 71 | Yes | Teams transport and persistence coverage: native card replay/expiry, route/thread identity, SDK boundary, terminal-card rendering, and focused recovery scenarios. Teams history hydration is N/A because no authenticated ordered history source exists. |
...
Slack, Discord, and Mattermost run **59 contract assertions** (16 ACL + 3 gateway + 11 routing policy + 16 session binding + 3 health + 10 shared prompt classification), plus 2 extra health assertions for snapshot-transport channels and 7 lifecycle assertions for channels with a gateway lifecycle actor. Teams runs the eight applicable lifecycle assertions in `TeamsSessionBindingContractTests`; its opaque Adaptive Card callback requires durable correlation and prompt-locator inputs that the text/button-shaped `SessionBindingContractTests` abstraction cannot represent safely. The focused Teams transport suite supplies the additional card, expiry/reissue, nonce, and replay coverage.
